...
export class ScrollBarDirective implements OnInit, DoCheck, OnDestroy, AfterViewInit {
    ...
    ngAfterViewInit() {
        ...
        if (this.alwaysShow) {
            ...
            $(this.elementRef.nativeElement).addClass('ps-active-x');
            $(this.elementRef.nativeElement).addClass('ps-active-y');
            $(this.elementRef.nativeElement).find('.ps-scrollbar-y-rail').find('.ps-scrollbar-y').css('opacity', 1);
            $(this.elementRef.nativeElement).find('.ps-scrollbar-x-rail').find('.ps-scrollbar-x').css('opacity', 1);
            this.lazyUpdate();
        } else {
