    ...
    return {
        ...
        module: {
            rules: [
                ...
                {
                    test: /\.bpmn/,
                    loaders: ['file-loader?hash=sha512&digest=hex&name=assets/bpmn/[hash].[ext]']
                },
                {
                    test: /\.ts$/,
        ...
        plugins: [
            ...
            new CopyWebpackPlugin([
                // tinymce相关开始
                {
                    context: path.resolve(`tinymce/langs/`),
                    from: '**/*',
                    to: path.resolve(`${config.dist}/langs/`)
                },
                {
                    context: path.resolve(`node_modules/tinymce/plugins/`),
                    from: '**/*',
                    to: path.resolve(`${config.dist}/plugins/`)
                },
                {
                    context: path.resolve(`node_modules/tinymce/themes/`),
                    from: '**/*',
                    to: path.resolve(`${config.dist}/themes/`)
                },
                {
                    context: path.resolve(`node_modules/tinymce/skins/`),
                    from: '**/*',
                    to: path.resolve(`${config.dist}/skins/`)
                },
                // tinymce相关结束
            ])
        ]
